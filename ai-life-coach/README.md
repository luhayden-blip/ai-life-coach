...
There is a **web version** of the same coach — no agent software needed, opens in any phone or desktop browser:

**👉 https://www.ailifecoach.cn**

Two honest notes: the two sides are **independent and do not sync automatically**; and if you have been using the skill, you can say "导出一下" to get a small structured summary (≤400 chars) and paste it into the web version's *Privacy & Security → bring your notes over* to continue. That transfer is **one-way**.

## Versioning

This is **v2.5.3** (unified version across all branches: web-version memory import + user-initiated "migration card" export (sentinel-delimited structured summary) + anti-injection declaration on the memory-injection path + a ClawHub-only "when to mention the web version" trigger section; on top of v2.4.2's cross-session complete journey cross-session complete journey — a "完整人生教练对话" walks four stages across sessions (four-dimension scoring → workview/lifeview → flow & strengths → three five-year versions), single-session summary + human-worded progress, blueprint has one unified entry (all four outputs gathered + consult + user consent, regardless of entry phrase); visible end-of-session summary + next-topic suggestion; farewell-strip regex extended. On top of v2.4.1's 抗挫与燃料五工具箱 — adversity-support four-step / six-pillar fragility diagnosis / courage fuel / entropy input audit / motivation fuel check — as black-box conversational tools; on top of v2.4.0's branch-3 web E2EE; on top of v2.3.0's intimate-relationship specialized module: cross-school integration · non-clinical coaching · China-context adaptation via `references/relationship.md`; on top of v2.2.7's hardened no-premature-farewell: the coach never ends the session on the user's behalf — a "收尾铁律" self-check plus a backend strip of any farewell phrasing; memory write decoupled from the end-summary marker; on top of v2.2.5's ending-power-to-user restraint + consultative blueprint; on top of v2.2.0's parent-child dual-perspective content layer `references/parent_child.md`; on top of v2.1.4's black-box principle + naturalness self-check + under-18 memory & abuse-handling refinement). Frontmatter `description` is bilingual; full English version is this README; full Chinese version is in `SKILL.md`.
